...
let AwsManager = function () {
    this.key = settings.get('S3_KEY');
    this.secret = settings.get('S3_SECRET');
    this.bucketName = settings.get('S3_BUCKET');
    aws.config.update({ accessKeyId: this.key, secretAccessKey: this.secret });
    this.S3 = new aws.S3();
};
